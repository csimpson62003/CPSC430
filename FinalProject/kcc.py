...
class PandaBulletCharacterController:
    """
    Adapted from https://github.com/jdfreder/panda3d-bullet-kcc/tree/master
    with minor modifications for the architecture we're using in CPSC 430
    ...
    def __init__(self, world, parent, game_object, gravity=None):
        """
        ...
        walkHeight = game_object.size[0]
        crouchHeight = game_object.size[1]
        stepHeight = game_object.size[2]
        radius = game_object.size[3]
    ...
    def __currentPos(self):
        return Vec3(*self.game_object.position)
    ...
    def __standUp(self):
        ...
        if self.__headContact is not None and self.__currentPos.z + self.__walkLevitation + self.__walkCapsuleH >= \
                self.__headContact[0].z:
            return
    ...
    def __processGround(self):
        if not self.isOnGround():
            ...
            self.setZ(self.__footContact[0].z)
    ...
    def __processJumping(self):
        ...
        self.setZ((self.gravity * self.jumpTime ** 2) + (self.jumpSpeed * self.jumpTime) + self.jumpStartPos)
    ...
    def __updateCapsule(self):
        ...
        self.game_object.position = self.__currentPos
    ...
    def setH(self, h):
        self.movementParent.setH(h)
        self.game_object.z_rotation = h

    def getH(self):
        return self.game_object.z_rotation

    def getP(self):
        return self.game_object.x_rotation

    def getR(self):
        return self.game_object.y_rotation

    def getPos(self):
        return self.game_object.position

    def getX(self):
        return self.game_object.position[0]

    def getY(self):
        return self.game_object.position[1]

    def getZ(self):
        return self.game_object.position[2]
